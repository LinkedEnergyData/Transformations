...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
		...
		xmlns:oldrdf="http://www.sparxsystems.com/profiles/RDF/1.0" xmlns:RDF="http://EAODM/schemas/RDF/_hlqRCzJoEeiH94jzXKzDEg/7"
		xmlns:oldowl="http://www.sparxsystems.com/profiles/OWL/1.0" xmlns:OWL="http://EAODM/schemas/OWL/_hlwXkDJoEeiH94jzXKzDEg/7"
	...
	<xsl:template match="/*">
        <xsl:element name="{name()}">
			...
			<xsl:namespace name="RDF" select="'http://EAODM/schemas/RDF/_hlqRCzJoEeiH94jzXKzDEg/7'"/>
			<xsl:namespace name="OWL" select="'http://EAODM/schemas/OWL/_hlwXkDJoEeiH94jzXKzDEg/7'"/>
			<xsl:attribute name="xsi:schemaLocation">http://EAODM/schemas/RDF/_hlqRCzJoEeiH94jzXKzDEg/7 /EA-ODM-PROFILE/EA-ODM.profile.uml#_hlqRDDJoEeiH94jzXKzDEg http://EAODM/schemas/OWL/_hlwXkDJoEeiH94jzXKzDEg/7 /EA-ODM-PROFILE/EA-ODM.profile.uml#_hlwXkTJoEeiH94jzXKzDEg</xsl:attribute>
            <xsl:apply-templates select="@*|node()" />
        </xsl:element>
    </xsl:template>
					...
					<xsl:otherwise>
						<xsl:element name="{name()}">
							...
							<xsl:element name="ownedComment">
								...
								<xsl:element name="body"><xsl:value-of select="$custclass/properties/@documentation"/></xsl:element>
							</xsl:element>
						</xsl:element>
					</xsl:otherwise>
				</xsl:choose>
					...
					<xsl:otherwise>
						<xsl:element name="{name()}">
							...
							<xsl:element name="ownedComment">
								<xsl:attribute name="xmi:id" select="concat(substring(./@xmi:id,1,6), 'comment', substring(./@xmi:id,14))"/>
								<xsl:attribute name="annotatedElement" select="./@xmi:id"/>
								<xsl:element name="body"><xsl:copy-of copy-namespaces="no" select="concat($custconn/source/documentation/@value,$custconn/target/documentation/@value)"/></xsl:element>
							</xsl:element>
						</xsl:element>
					</xsl:otherwise>
				</xsl:choose>
